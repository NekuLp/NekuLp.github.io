...
function draw() {   
  ...
  seno = (sin(radians(frameCount)) * 900);
  
  endSize = map(seno, 0, max(width, 1100), tileWidth / 2, 0);
  endOffset = map(0, 0, max(height, 0), 0, (tileWidth - endSize) / 2);
  ...
  for (var gridY = 0; gridY <= tileCountY; gridY++) {
    for (var gridX = 0; gridX <= tileCountX; gridX++) {
      ...
      for (var i = 0; i < circleCount; i++) {
        ...
        square(offset, diameter, diameter);
  ...
  console.log('MouseX '+endSize);
  console.log('MouseY '+endOffset);
}
