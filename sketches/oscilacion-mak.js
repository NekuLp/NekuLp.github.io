 ...
 */
 ...
 var sketch = function(p) {
   ...
   p.setup = function() {
    ...
    p.noFill();
    p.frameRate(60); // Adjust the frame rate as needed
  
    lineColor = p.color(0, 50);
  };

  p.draw = function() {
    // Animate phi back and forth over time
    phi = p.sin(p.frameCount * 0.01) * 180; // back-and-forth from -180 to +180
   ...
   p.keyPressed = function() {
     if (p.key == 's' || p.key == 'S') p.saveCanvas('LissaJous', 'png');
